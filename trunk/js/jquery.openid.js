...
jQuery.fn.openid = function(opt) {
  ...
  var signIn = function(obj, tidx) {
    ...
    if (gprovider.label === null) {
      $('#openid_inputarea').text(settings.txt.title);
      if (!tidx) {
        $('#openid_inputarea').fadeOut();
        form.submit();
      }
    }
    return false;
  ...
  var showInputBox = function() {
    ...
    $('#openid_inputarea').empty().show().append('<span class="oidlabel">' + lbl + '</span><input id="' + INPUTID + '" type="text" ' +
  ...
  var form = this;
  var inputarea = $('#openid_inputarea');
  if (!inputarea.length) inputarea = $('<div id="openid_inputarea" />');
